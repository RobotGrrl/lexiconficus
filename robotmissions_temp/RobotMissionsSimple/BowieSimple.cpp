...
void Bowie::init() {
  ...
  //gyro.enableAutoRange(true); // enable auto-ranging on gyro

  if(!accel.begin()) {
    Serial << "ERROR: Accel was unable to begin" << endl;
    ACCEL_ENABLED = false;
  } else {
    ACCEL_ENABLED = true;
  }
  ...
  if(!mag.begin()) {
    Serial << "ERROR: Mag was unable to begin" << endl;
    MAG_ENABLED = false;
  } else {
    MAG_ENABLED = true;
  }
  ...
  if(!bmp.begin()) {
    Serial << "ERROR: BMP was unable to begin" << endl;
    BMP_ENABLED = false;
  } else {
    BMP_ENABLED = true;
  }
  ...
  if(!gyro.begin()) {
    Serial << "ERROR: Gyro was unable to begin" << endl;
    GYRO_ENABLED = false;
  } else {
    GYRO_ENABLED = true;
  }
...
void Bowie::update() {
  ...
  if(GYRO_ENABLED) {
    ...
    if(abs(gx) <= 100 && abs(gy) <= 100 && abs(gz) <= 100) {
      gyro_msg_x = (int)(abs(gx)*100);
      if(gx < 0) gyro_msg_x += 9000;

      gyro_msg_y = (int)(abs(gy)*100);
      if(gy < 0) gyro_msg_y += 9000;

      gyro_msg_z = (int)(abs(gz)*100);
      if(gz < 0) gyro_msg_z += 9000;
    }
  ...
  if(MAG_ENABLED) {
    ...
    if(abs(mx) <= 100 && abs(my) <= 100 && abs(mz) <= 100) {

      mag_msg_x = (int)(abs(mx)*10);
      if(mx < 0) mag_msg_x += 9000;
      
      mag_msg_y = (int)(abs(my)*10);
      if(my < 0) mag_msg_y += 9000;

      mag_msg_z = (int)(abs(mz)*10);
      if(mz < 0) mag_msg_z += 9000;
  ...
  if(ACCEL_ENABLED) {
    ...
    if(abs(ax) <= 100 && abs(ay) <= 100 && abs(az) <= 100) {
      ...
      accel_msg_x = (int)(abs(ax)*10);
      if(ax < 0) accel_msg_x += 9000;
      
      accel_msg_y = (int)(abs(ay)*10);
      if(ay < 0) accel_msg_y += 9000;

      accel_msg_z = (int)(abs(az)*10);
      if(az < 0) accel_msg_z += 9000;
  ...
  if(BMP_ENABLED) {
    ...
    float alt = bmp.pressureToAltitude(seaLevelPressure, bmp_event.pressure, temperature);

    alt_msg = abs( (int)(alt) );
    if(alt < 0) alt_msg += 9000;

    temp_msg = abs( (int)(temperature) );
    if(temperature < 0) temp_msg += 9000;
...
void Bowie::initServos() {
  ...
  pinMode(SERVO_END, OUTPUT);
  pinMode(SERVO_OTHER, OUTPUT);
...
void Bowie::initLeds() {
  ...
  pinMode(BRIGHT_LED_LEFT, OUTPUT);
  pinMode(BRIGHT_LED_RIGHT, OUTPUT);
